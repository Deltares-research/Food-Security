...
def get_crop_info(
    ...
    try:
        crop_fao_price = fao_mapping_price_df[
            fao_mapping_price_df["fao_flc"] == crop_fao
        ]["fao_producer"].iloc[0]
    except IndexError:
        crop_fao_price = None
...
def get_producer_prices(pp_df: pd.DataFrame, crop_name: str):
    pp_df_sorted = pp_df.sort_values(by="Year")
    pp_crop = pp_df_sorted[
        (pp_df_sorted["Item"] == crop_name)
